...
export default function GlobeBackground() {
  ...
  useEffect(() => {
    ...
    const coreGlobe = new THREE.Mesh(
      ...
      new THREE.MeshPhongMaterial({ color: 0x020818, transparent: true, opacity: 0.1 })
    )
    scene.add(coreGlobe)
    ...
    const middleGlobe = new THREE.Mesh(new THREE.SphereGeometry(5, 32, 32), middleMaterial)
    ...
    // --- Color Animation ---
    const colors = [0x01e4f8, 0x0a84ff, 0x6b46c1, 0x01e4f8].map(c => new THREE.Color(c))
    let colorIndex = 0
    ...
    const animate = () => {
      ...
      const baseColor = lerpColor(colors[colorIndex], colors[nextColorIndex], colorT)

      pulseT += pulseSpeed
      const pulseFactor = (Math.sin(pulseT) + 1) / 2
      const finalColor = baseColor.clone().multiplyScalar(0.5 + pulseFactor * 0.5)
    ...
    const handleResize = () => {
      ...
      // تنظیم فاصله دوربین بر اساس سایز صفحه
  ...
  return (
    <div ref={mountRef} className="fixed inset-0 -z-10 w-full h-full overflow-hidden">
