import LinearProgress from "../../../components/elements/LinearProgress";
import React from "react";

export default function LevelsBar() {
  return (
    <div className="w-full flex flex-col items-center p-2 gap border-2 border-[#31313a] rounded-[10px] bg-black/30">
      <span className="text-sm font-medium text-[#DCDEE0] w-full text-start">
        سطح شما 1 از سطح فلان
      </span>
      <div className="w-full flex flex-row-reverse items-center justify-between">
        <LinearProgress percent={35} />
        <span className="text-[8px] font-extrabold text-nowrap text-sky-400">درصد پیشرفت</span>
      </div>
      <div className=""></div>
    </div>
  );
}
